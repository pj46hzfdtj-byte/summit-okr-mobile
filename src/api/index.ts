...
export const authApi = {
  ...
  refresh: (dto: RefreshTokenDto) => http.post<AuthTokens>('/auth/refresh', dto, { silent: true }),
...
export const keyResultApi = {
  listByObjective: (objectiveId: string) =>
    http.get<KeyResult[]>(`/key-results/by-objective/${objectiveId}`),
  create: (dto: CreateKeyResultDto) => http.post<KeyResult>('/key-results', dto),
  update: (id: string, dto: UpdateKeyResultDto) => http.patch<KeyResult>(`/key-results/${id}`, dto),
...
export const memoApi = {
  list: (ownerType: string, ownerId: string) =>
    http.get<Memo[]>('/memos', { params: { ownerType, ownerId } }),
...
export const taskApi = {
  ...
  complete: (id: string, completed: boolean) =>
    http.post<Task>(`/tasks/${id}/complete`, { completed }),
...
export const focusCycleApi = {
  ...
  update: (id: string, dto: UpdateFocusCycleDto) =>
    http.patch<FocusCycle>(`/focus-cycles/${id}`, dto),
...
export const ganttApi = {
  get: (params: { scope?: 'all' | 'cycle'; goalGroupId?: string }) =>
    http.get<GanttData>('/gantt', { params }),
};

// ============ Data Export/Import ============
export const dataApi = {
  /** 移动端直接取 JSON 文本（web 端为 blob 下载） */
  export: () => http.get<any>('/data/export'),
...
export const feedbackApi = {
  create: (dto: { type: string; content: string; contact?: string }) =>
    http.post<Feedback>('/feedback', dto),
...
export const aiApi = {
  ...
  suggestScore: (objectiveId: string) =>
    http.post<AiSuggestScoreResult>('/ai/suggest-score', { objectiveId }),
...
export const checkinApi = {
  ...
  upsertThisWeek: (note?: string) => http.put<CheckIn>('/checkins/this-week', { note }),
